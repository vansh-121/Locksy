...
│   │   ├── intruder-log.html     # Intruder photo log viewer (NEW in v2.6.0)
│   │   ├── keyboard-shortcuts.html # Keyboard shortcuts guide
│   │   ├── locked.html             # Locked tab overlay interface
│   │   ├── popup.html              # Extension popup interface
│   │   ├── privacy-report.html     # Privacy Report dashboard (NEW in v3.0.0)
...
│       ├── intruder-log.js       # Intruder photo log page controller (NEW in v2.6.0)
│       ├── keyboard-shortcuts.js   # Keyboard shortcuts guide logic
│       ├── license-manager.js      # Polar.sh license manager (NEW in v3.0.0)
│       ├── locked.js               # Locked tab overlay logic (incl. biometric unlock, intruder capture)
│       ├── popup.js                # Popup UI logic and event handlers
│       ├── privacy-report.js       # Privacy Report dashboard logic (NEW in v3.0.0)
...
│
├── locksy-api/                # Cloudflare Worker API for Polar licensing & rate limiting (NEW in v3.0.0)
│
├── tests/                     # Testing files
│   ├── performance-dashboard.html # Performance monitoring UI
│   ├── performance-test.js   # PBKDF2 performance testing
│   ├── popup-preview.html    # UI popup preview utility (NEW in v3.0.0)
│   ├── test-kdf.html         # KDF testing interface
│   └── README.md             # Testing documentation
│
├── .github/                   # GitHub workflows
│   └── workflows/
│       ├── auto-release.yml   # Auto tag + GitHub release on push to main
│       ├── build.yml          # CI build of Chrome/Firefox packages (push & PR)
│       ├── release.yml        # Release build on v* tag or manual dispatch
│       └── safari-build.yml   # Safari (macOS) conversion + build artifact
│
├── .git/                      # Git repository data
├── .gitignore                # Git ignore rules
├── build.js                  # Build script for distribution packages (with Terser minification)
...
- **RELEASE_NOTES_v2.5.0.txt**: Detailed release notes for version 2.5.0 (context menus, stealth mode)
- **RELEASE_NOTES_v2.6.0.txt**: Detailed release notes for version 2.6.0 (intruder detection)
- **RELEASE_NOTES_v2.7.0.txt**: Detailed release notes for version 2.7.0 (startup lock)
- **RELEASE_NOTES_v3.0.0.txt**: Detailed release notes for version 3.0.0 (redesign, license verification, privacy reports)
...
- **background.js**: Service worker that manages locked tabs, domain locks, auto-lock timers, scheduled locking with Chrome Alarms API, right-click context menus, stealth mode, activity tracking, keyboard shortcut handlers, badge updates, intruder photo storage (`saveIntruderPhoto`, `getIntruderPhotos`, `clearIntruderPhotos` message handlers), and monitors tab events
...
- **intruder-log.js**: Intruder Log page controller — `renderPhotos()`, `deletePhoto()`, and bulk-delete via `getIntruderPhotos`/`clearIntruderPhotos` background messages (NEW in v2.6.0)
- **keyboard-shortcuts.js**: Keyboard shortcuts guide page logic and navigation
- **license-manager.js**: Polar.sh license client management logic, key activation, integrity token verification, and offline grace tracking (NEW in v3.0.0)
- **locked.js**: Locked tab overlay logic with password verification, biometric (WebAuthn) unlock (including biometric-default mode and visibility-aware auto-trigger), intruder photo capture (`captureIntruderPhoto()`), rate limiting, and unlock functionality
- **popup.js**: Extension popup UI logic including password management, tab locking controls, biometric settings, auto-lock timer settings, scheduled locking configuration, stealth mode toggle, locked tabs quick-unlock panel, intruder detection toggle, weekly privacy report entry, license state widget, developer information, and sponsor button
- **privacy-report.js**: Controller for the Privacy Report page, processing security scores and rendering charts/history tables (NEW in v3.0.0)
...
- **intruder-log.html**: Intruder photo log viewer — grid layout with timestamps, per-card delete, and bulk delete (NEW in v2.6.0)
...
- **popup.html**: Extension popup interface
- **privacy-report.html**: Weekly Privacy Report visual dashboard and activity tracker (NEW in v3.0.0)
...
- **popup-preview.html**: Extension popup layout and styling preview page (NEW in v3.0.0)
...
### Functionality
- **Startup Lock**: Auto-lock all tabs on browser start via `chrome.runtime.onStartup`
  - Session guard (`startupLockApplied`) prevents re-locking mid-session
  - Works with browser "Continue where you left off" session restore
  - New 🚀 toggle in popup settings panel
...
- **Intruder Detection**: Webcam photo captured on 3rd+ failed unlock attempt; stored locally only (NEW in v2.6.0)
- **Locked Tabs Quick-Unlock Panel**: Popup shows all locked tabs with one-click unlock — no password re-entry (NEW in v2.6.0)
- **Biometric-Default Mode**: Set fingerprint/face as the primary unlock method — auto-triggers on locked pages (NEW in v2.6.0)
